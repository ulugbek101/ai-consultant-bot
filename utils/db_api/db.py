...
class Database:
    ...
    _SUBCATEGORY_SEED = [
        ...
        ("svc_accounting", "Учёт по НСБУ / НСФО",                    1),
        ("svc_accounting", "Учёт по МСФО",                            2),
        ("svc_accounting", "Восстановление учёта",                    3),
        ("svc_accounting", "Импорт и экспорт",                        4),
        # 2. НАЛОГОВОЕ КОНСУЛЬТИРОВАНИЕ
        ("svc_tax",        "Выбор системы налогообложения",           1),
        ("svc_tax",        "Налоговая отчётность",                    2),
        ("svc_tax",        "Требования налоговых органов",            3),
        ("svc_tax",        "Налоговый аудит",                         4),
        # 3. ЮРИДИЧЕСКАЯ КОНСУЛЬТАЦИЯ
        ("svc_legal",      "Регистрация ООО, СП, ЯТТ",               1),
        ("svc_legal",      "Экспертиза договоров",                    2),
        ("svc_legal",      "Работа с должниками",                     3),
        ("svc_legal",      "Юридические консультации",                4),
        ("svc_legal",      "Трудовое законодательство",               5),
        # 4. АУДИТ
        ("svc_audit",      "Обязательный / инициативный аудит",       1),
        ("svc_audit",      "Аудит для IT Park",                       2),
        ("svc_audit",      "Крупные и аффилированные сделки",         3),
        ("svc_audit",      "Трансформация по МСФО",                   4),
        # 5. КОНСУЛЬТАЦИИ ПО КРЕДИТАМ И ИНВЕСТИЦИЯМ
        ("svc_credit",     "Кредиты и льготные программы",            1),
        ("svc_credit",     "Бизнес-планы и ТЭО",                      2),
        # 6. ФИНАНСОВЫЙ КОНСАЛТИНГ ПО ДРУГИМ ВОПРОСАМ
        ("svc_finance",    "Корпоративное управление",                1),
        ("svc_finance",    "Учредители и инвесторы",                  2),
        ("svc_finance",    "Государственные закупки",                 3),
        ("svc_finance",    "Финансово-хоз. деятельность",             4),
        # 7. КАДРОВЫЙ УЧЕТ
        ("svc_hr",         "Кадровый учёт",                           1),
        ("svc_hr",         "Трудовое законодательство",               2),
    ]
    ...
    def update_subcategory_labels(self) -> None:
        for category_key, label, sort_order in self._SUBCATEGORY_SEED:
            self.execute(
                "UPDATE subcategories SET label = %s WHERE category_key = %s AND sort_order = %s",
                (label, category_key, sort_order), commit=True,
            )
